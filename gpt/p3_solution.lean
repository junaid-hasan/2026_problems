...
lemma Refines.length_le {xs ys : List ℝ} (h : Refines xs ys) :
    xs.length ≤ ys.length := by
  induction h with
  | nil => simp
  | cons u hu hsum hrest ih =>
      rw [List.length_cons, List.length_append]
      have hu0 : u.length ≠ 0 := fun hzero => hu (List.eq_nil_of_length_eq_zero hzero)
...
lemma pieceLengths_refines_of_subset {S T : Finset ℝ}
  ...
  · rw [pieceLengths_sum, pieceLengths_sum]
  · intro z hz
...
def alternatingLoss : List ℝ → ℝ
  | [] => 0
  | _a :: [] => 0
  | _a :: b :: t => b + alternatingLoss t
...
lemma alternatingShare_add_alternatingLoss (l : List ℝ) :
    alternatingShare l + alternatingLoss l = l.sum := by
  induction l using List.twoStepInduction with
  | nil => simp [alternatingShare, alternatingLoss]
  | singleton a => simp [alternatingShare, alternatingLoss]
  | cons_cons a b t ih =>
      simp only [alternatingShare, alternatingLoss, List.sum_cons]
      linarith

lemma alternatingShare_sub_alternatingLoss (l : List ℝ) :
    alternatingShare l - alternatingLoss l = alternatingDiscrepancy l := by
  induction l using List.twoStepInduction with
  | nil => simp [alternatingShare, alternatingLoss, alternatingDiscrepancy]
  | singleton a => simp [alternatingShare, alternatingLoss, alternatingDiscrepancy]
  | cons_cons a b t ih =>
      simp only [alternatingShare, alternatingLoss, alternatingDiscrepancy]
      linarith

lemma twice_alternatingShare (l : List ℝ) :
    2 * alternatingShare l = l.sum + alternatingDiscrepancy l := by
  linarith [alternatingShare_add_alternatingLoss l,
    alternatingShare_sub_alternatingLoss l]

lemma alternatingDiscrepancy_nonneg_of_pairwise
    {l : List ℝ} (hsort : l.Pairwise (· ≥ ·))
    (hnonneg : ∀ x ∈ l, 0 ≤ x) :
    0 ≤ alternatingDiscrepancy l := by
  induction l using List.twoStepInduction with
  | nil => simp [alternatingDiscrepancy]
  | singleton a =>
      simp only [alternatingDiscrepancy]
      exact hnonneg a (by simp)
  | cons_cons a b t ih =>
      simp only [alternatingDiscrepancy]
      have hab : b ≤ a := by
        simpa using (List.pairwise_cons.mp hsort).1 b (by simp)
      have ht : t.Pairwise (· ≥ ·) :=
...
lemma firstPlayerShare_ge_half_sum (l : List ℝ)
    (hnonneg : ∀ x ∈ l, 0 ≤ x) :
    l.sum / 2 ≤ firstPlayerShare l := by
  let s := l.mergeSort (· ≥ ·)
  have hsperm : s.Perm l := List.mergeSort_perm l (· ≥ ·)
  have hsn : ∀ x ∈ s, 0 ≤ x := by
    intro x hx
    exact hnonneg x (hsperm.mem_iff.mp hx)
  have hsort : s.Pairwise (· ≥ ·) := by
    have hs := List.pairwise_mergeSort (l := l) (le := fun a b : ℝ => decide (a ≥ b))
      (by
        intro a b c hab hbc
  ...
  have hd := alternatingDiscrepancy_nonneg_of_pairwise hsort hsn
  rw [firstPlayerShare_eq_alternatingShare]
  have hsum : s.sum = l.sum := hsperm.sum_eq
  have htwice := twice_alternatingShare s
  dsimp [s] at htwice hd hsum ⊢
  linarith
...
lemma p3_pieceLengths_length (S : Finset ℝ) :
    (P3Realization.pieceLengths S).length = S.card + 1 := by
  simp [P3Realization.pieceLengths]
...
lemma labelTotal_eq_realization
    {α : Type*} [DecidableEq α] (a : α) (xs : List ℝ) (labels : List α) :
    labelTotal a xs labels = P3Realization.labelTotal a xs labels := by
  induction xs generalizing labels with
  | nil => simp [labelTotal, P3Realization.labelTotal]
  | cons x xs ih =>
      cases labels with
      | nil => simp [labelTotal, P3Realization.labelTotal]
      | cons b labels =>
          simp only [labelTotal, P3Realization.labelTotal]
          rw [ih]
...
theorem lower_bound_aux (n : ℕ) (hn : 0 < n) :
  ...
  have hTint : ↑T ⊆ Set.Ioo (0 : ℝ) 1 := by
    ...
    · exact hAint hx
    · exact hB.1 hx
  have hcoarse : P3Realization.pieceLengths A = binaryGaps n := by
    dsimp [A]
    ...
    · exact hgapPos
    · exact hgapSum
  have hrefine : P3Realization.Refines (binaryGaps n)
      (P3Realization.pieceLengths T) := by
    rw [← hcoarse]
  ...
  let ys : List ℝ := P3Realization.pieceLengths T
  ...
  have hsortedNonneg : ∀ x ∈ sorted, 0 ≤ x := by
    ...
    exact le_of_lt (P3Realization.pieceLengths_pos T hTint x (by
  ...
  have hsortedCard : sorted.length ≤ 2 * (n + 1) - 1 := by
    have hylen : ys.length = T.card + 1 := by
      exact p3_pieceLengths_length T
  ...
  have hsortedSumOne : sorted.sum = 1 := by
    calc
      ...
      _ = 1 := by exact P3Realization.pieceLengths_sum T
  have hsortedSum :
  ...
  have hsortedGroup : ∀ i : Fin (n + 1),
    ...
    rw [hmap]
    rw [labelTotal_eq_realization]
    change P3Realization.labelTotal (labelEquiv.symm i)
      (P3Realization.pieceLengths T) labels = _
  ...
  have hShare : binaryDelta n * (2 : ℝ) ^ n ≤ firstPlayerShare ys := by
    rw [firstPlayerShare_eq_alternatingShare]
    exact hAlt
  simpa [L, ys, T, pieceLengths, P3Realization.pieceLengths,
    binaryDelta, div_eq_mul_inv, mul_comm] using hShare
...
lemma selectedSum_append (n : ℕ) (l₁ l₂ : List ℝ) :
    selectedSum n (l₁ ++ l₂) =
      selectedSum n l₁ + selectedSum (n + l₁.length) l₂ := by
  induction l₁ generalizing n with
  | nil => simp [selectedSum]
  | cons x l₁ ih =>
      simp only [List.cons_append, selectedSum_cons, List.length_cons]
      rw [ih]
      rw [show n + 1 + l₁.length = n + (l₁.length + 1) by omega]
      ring
...
def pairedResidual (u : List ℝ) (r : ℝ) (v : List ℝ) : List ℝ :=
  pairDup u ++ r :: pairDup v

lemma selectedSum_pairedResidual (n : ℕ) (u : List ℝ) (r : ℝ) (v : List ℝ) :
    selectedSum n (pairedResidual u r v) =
      u.sum + (if n % 2 = 0 then r else 0) + v.sum := by
  induction u generalizing n with
  | nil =>
      change selectedSum n (r :: pairDup v) =
        [].sum + (if n % 2 = 0 then r else 0) + v.sum
      rw [selectedSum_cons, selectedSum_pairDup]
      simp
  | cons x u ih =>
      change selectedSum n (x :: x :: pairedResidual u r v) =
        (x :: u).sum + (if n % 2 = 0 then r else 0) + v.sum
      rw [selectedSum_cons, selectedSum_cons, ih]
      by_cases hn : n % 2 = 0
      · have hn1 : (n + 1) % 2 ≠ 0 := by omega
...
lemma mergeSort_eq_pairedResidual {L u v : List ℝ} {r : ℝ}
    (hperm : L.Perm (pairedResidual u r v))
    (hsorted : (pairedResidual u r v).Pairwise (· ≥ ·)) :
    L.mergeSort (· ≥ ·) = pairedResidual u r v := by
  have hp : List.Perm (L.mergeSort (· ≥ ·)) (pairedResidual u r v) :=
    (List.mergeSort_perm L _).trans hperm
  exact hp.eq_of_pairwise' (List.pairwise_mergeSort' (· ≥ ·) L) hsorted

lemma firstPlayerShare_pairedResidual {L u v : List ℝ} {r : ℝ}
    (hperm : L.Perm (pairedResidual u r v))
    (hsorted : (pairedResidual u r v).Pairwise (· ≥ ·)) :
    firstPlayerShare L = u.sum + r + v.sum := by
  unfold firstPlayerShare
  rw [mergeSort_eq_pairedResidual hperm hsorted]
  change selectedSum 0 (pairedResidual u r v) = u.sum + r + v.sum
  simpa using selectedSum_pairedResidual 0 u r v

lemma sum_pairedResidual (u : List ℝ) (r : ℝ) (v : List ℝ) :
    (pairedResidual u r v).sum = 2 * u.sum + r + 2 * v.sum := by
  simp [pairedResidual, sum_pairDup]
  ring

lemma firstPlayerShare_residual_total {L u v : List ℝ} {r : ℝ}
    (hperm : L.Perm (pairedResidual u r v))
    (hsorted : (pairedResidual u r v).Pairwise (· ≥ ·)) :
    firstPlayerShare L = (L.sum + r) / 2 := by
  rw [firstPlayerShare_pairedResidual hperm hsorted, hperm.sum_eq,
    sum_pairedResidual]
  ring

lemma firstPlayerShare_residual_of_sum_one {L u v : List ℝ} {r : ℝ}
    (hperm : L.Perm (pairedResidual u r v))
    (hsorted : (pairedResidual u r v).Pairwise (· ≥ ·))
    (hsum : L.sum = 1) : firstPlayerShare L = (1 + r) / 2 := by
  rw [firstPlayerShare_residual_total hperm hsorted, hsum]

lemma firstPlayerShare_half_le_of_residual {L u v : List ℝ} {r : ℝ}
    (hperm : L.Perm (pairedResidual u r v))
    (hsorted : (pairedResidual u r v).Pairwise (· ≥ ·))
    (hsum : L.sum = 1) (hr : 0 ≤ r) : (1 : ℝ) / 2 ≤ firstPlayerShare L := by
  rw [firstPlayerShare_residual_of_sum_one hperm hsorted hsum]
  linarith

lemma pairedResidual_pairwise {u v : List ℝ} {r : ℝ}
    (hu : u.Pairwise (· ≥ ·)) (hv : v.Pairwise (· ≥ ·))
    (hur : ∀ x ∈ u, x ≥ r) (hrv : ∀ y ∈ v, r ≥ y) :
    (pairedResidual u r v).Pairwise (· ≥ ·) := by
  unfold pairedResidual
  rw [List.pairwise_append]
  refine ⟨pairDup_pairwise hu, ?_, ?_⟩
  · rw [List.pairwise_cons]
    refine ⟨?_, pairDup_pairwise hv⟩
    intro y hy
    exact hrv y (mem_pairDup.mp hy)
  · intro x hx y hy
    have hxu : x ∈ u := mem_pairDup.mp hx
    simp only [List.mem_cons] at hy
    rcases hy with rfl | hy
    · exact hur x hxu
    · exact (hrv y (mem_pairDup.mp hy)).trans (hur x hxu)
...
lemma selectedSum_add_two (l : List ℝ) (k : ℕ) :
    selectedSum (k + 2) l = selectedSum k l := by
  induction l generalizing k with
  | nil => rfl
  | cons a t ih =>
      rw [selectedSum_cons, selectedSum_cons]
...
lemma selectedSum_zero_eq_alternatingShare (l : List ℝ) :
    selectedSum 0 l = alternatingShare l := by
  induction l using List.twoStepInduction with
  | nil => rfl
  | singleton a => simp [selectedSum, alternatingShare]
  | cons_cons a b t ih =>
      rw [selectedSum_cons, selectedSum_cons]
      rw [selectedSum_add_two t 0, ih]
      simp [alternatingShare]

lemma firstPlayerShare_eq_alternatingShare (l : List ℝ) :
    firstPlayerShare l = alternatingShare (l.mergeSort (· ≥ ·)) := by
  unfold firstPlayerShare
  change selectedSum 0 (l.mergeSort (· ≥ ·)) = _
  exact selectedSum_zero_eq_alternatingShare _
...
def pickedSum (l : List ℝ) (i : ℕ := 0) : ℝ :=
  (((l.zipIdx i).filter (fun p => p.2 % 2 = 0)).map (fun p => p.1)).sum

lemma pickedSum_nil (i : ℕ) : pickedSum [] i = 0 := by
  simp [pickedSum]

lemma pickedSum_pair (x y : ℝ) (l : List ℝ) (i : ℕ)
    (hi : i % 2 = 0) :
    pickedSum (x :: y :: l) i = x + pickedSum l (i + 2) := by
  have hi1 : (i + 1) % 2 = 1 := by omega
  simp [pickedSum, List.zipIdx_cons, hi, hi1, add_assoc]

lemma pickedSum_singleton (x : ℝ) (i : ℕ) (hi : i % 2 = 0) :
    pickedSum [x] i = x := by
  simp [pickedSum, List.zipIdx_cons, hi]

lemma pickedSum_cons_even (x : ℝ) (l : List ℝ) (i : ℕ)
    (hi : i % 2 = 0) :
    pickedSum (x :: l) i = x + pickedSum l (i + 1) := by
  simp [pickedSum, List.zipIdx_cons, hi]

lemma pickedSum_eq_firstPlayerShare (l : List ℝ)
    (hs : List.Pairwise (· ≥ ·) l) :
    firstPlayerShare l = pickedSum l := by
  rw [firstPlayerShare, List.mergeSort_eq_self _ hs]
  rfl

lemma pickedSum_double_pair (x : ℝ) (l : List ℝ) (i : ℕ) :
    pickedSum (x :: x :: l) i = x + pickedSum l (i + 2) := by
  have hi : i % 2 = 0 ∨ i % 2 = 1 := by omega
  rcases hi with hi | hi
  · exact pickedSum_pair x x l i hi
  · have hi1 : (i + 1) % 2 = 0 := by omega
    simp [pickedSum, List.zipIdx_cons, hi, hi1, add_assoc]

def doubleList : List ℝ → List ℝ
  | [] => []
  | x :: xs => x :: x :: doubleList xs

@[simp] lemma doubleList_sum (q : List ℝ) :
    (doubleList q).sum = 2 * q.sum := by
  induction q with
  | nil => simp [doubleList]
  | cons x xs ih => simp [doubleList, ih]; ring

@[simp] lemma mem_doubleList {z : ℝ} {q : List ℝ} :
    z ∈ doubleList q ↔ z ∈ q := by
  induction q with
  | nil => simp [doubleList]
  | cons x xs ih => simp [doubleList, ih]

lemma doubleList_pairwise {q : List ℝ}
    (hq : List.Pairwise (· ≥ ·) q) :
    List.Pairwise (· ≥ ·) (doubleList q) := by
  induction q with
  | nil => simp [doubleList]
  | cons x xs ih =>
      rw [List.pairwise_cons] at hq
      rw [doubleList, List.pairwise_cons]
      constructor
      · intro z hz
        simp only [List.mem_cons] at hz
        rcases hz with rfl | hz
        · exact le_rfl
        · exact hq.1 z (mem_doubleList.mp hz)
      · rw [List.pairwise_cons]
        exact ⟨fun z hz => hq.1 z (mem_doubleList.mp hz), ih hq.2⟩

lemma pickedSum_doubleList (q : List ℝ) (i : ℕ) :
    pickedSum (doubleList q) i = q.sum := by
  induction q generalizing i with
  | nil => simp [doubleList, pickedSum]
  | cons x xs ih =>
      rw [doubleList, pickedSum_double_pair, ih]
      simp

/-- Insert one unpaired residual into a descending list whose entries will all
be duplicated.  The result is still descending. -/
noncomputable def pairResidual (r : ℝ) : List ℝ → List ℝ
  | [] => [r]
  | x :: xs =>
      if r ≥ x then r :: doubleList (x :: xs)
      else x :: x :: pairResidual r xs

@[simp] lemma mem_pairResidual {z r : ℝ} {q : List ℝ} :
    z ∈ pairResidual r q ↔ z = r ∨ z ∈ q := by
  induction q with
  | nil => simp [pairResidual]
  | cons x xs ih =>
      by_cases h : r ≥ x
      · simp [pairResidual, h]
      · simp only [pairResidual, if_neg h, List.mem_cons, ih]
        aesop

lemma pairResidual_perm (r : ℝ) (q : List ℝ) :
    (pairResidual r q).Perm (r :: doubleList q) := by
  induction q with
  | nil => simp [pairResidual, doubleList]
  | cons x xs ih =>
      by_cases h : r ≥ x
      · simp [pairResidual, doubleList, h]
      · rw [pairResidual, if_neg h, doubleList]
        exact (List.Perm.cons x (List.Perm.cons x ih)).trans
          ((List.Perm.cons x (List.Perm.swap r x (doubleList xs))).trans
            (List.Perm.swap r x (x :: doubleList xs)))

lemma pairResidual_pairwise (r : ℝ) {q : List ℝ}
    (hq : List.Pairwise (· ≥ ·) q) :
    List.Pairwise (· ≥ ·) (pairResidual r q) := by
  induction q with
  | nil => simp [pairResidual]
  | cons x xs ih =>
      rw [List.pairwise_cons] at hq
      by_cases h : r ≥ x
      · rw [pairResidual, if_pos h, List.pairwise_cons]
        constructor
        · intro z hz
          have hzq : z ∈ x :: xs := mem_doubleList.mp hz
          simp only [List.mem_cons] at hzq
          rcases hzq with rfl | hzq
          · exact h
          · exact le_trans (hq.1 z hzq) h
        · exact doubleList_pairwise (List.pairwise_cons.mpr ⟨hq.1, hq.2⟩)
      · have hxr : x ≥ r := le_of_lt (lt_of_not_ge h)
        rw [pairResidual, if_neg h, List.pairwise_cons]
        constructor
        · intro z hz
          simp only [List.mem_cons] at hz
          rcases hz with rfl | hz
          · exact le_rfl
          · rcases mem_pairResidual.mp hz with rfl | hz
            · exact hxr
            · exact hq.1 z hz
        · rw [List.pairwise_cons]
          constructor
...
lemma pickedSum_pairResidual (r : ℝ) (q : List ℝ) (i : ℕ)
    (hi : i % 2 = 0) :
    pickedSum (pairResidual r q) i = r + q.sum := by
  induction q generalizing i with
  | nil => simpa [pairResidual] using pickedSum_singleton r i hi
  | cons x xs ih =>
      by_cases h : r ≥ x
      · rw [pairResidual, if_pos h]
        rw [pickedSum_cons_even r (doubleList (x :: xs)) i hi,
          pickedSum_doubleList]
      · rw [pairResidual, if_neg h, pickedSum_double_pair,
          ih (i + 2) (by omega)]
        simp only [List.sum_cons]
        ring

lemma doubleList_eq_flatMap (q : List ℝ) :
    doubleList q = q.flatMap (fun x => [x, x]) := by
  induction q with
  | nil => rfl
  | cons x xs ih => simp [doubleList, ih]

lemma doubleList_perm {q₁ q₂ : List ℝ} (h : q₁.Perm q₂) :
    (doubleList q₁).Perm (doubleList q₂) := by
  rw [doubleList_eq_flatMap, doubleList_eq_flatMap]
  exact h.flatMap (fun _ _ => List.Perm.refl _)

/-- Sorting a multiset consisting of duplicated entries and one residual leaves
exactly the residual as the alternating-sum imbalance. -/
lemma firstPlayerShare_of_perm_residual_pairs (l q : List ℝ) (r : ℝ)
    (hl : l.Perm (r :: doubleList q)) :
    firstPlayerShare l = r + q.sum := by
  let qs := q.mergeSort (· ≥ ·)
  have hqs : List.Pairwise (· ≥ ·) qs :=
    List.pairwise_mergeSort' (· ≥ ·) q
  have hqqs : q.Perm qs := (List.mergeSort_perm q (· ≥ ·)).symm
  have hperm : (l.mergeSort (· ≥ ·)).Perm (pairResidual r qs) :=
    (List.mergeSort_perm l (· ≥ ·)).trans <|
      hl.trans <| (List.Perm.cons r (doubleList_perm hqqs)).trans <|
        (pairResidual_perm r qs).symm
  have heq : l.mergeSort (· ≥ ·) = pairResidual r qs :=
    hperm.eq_of_pairwise'
      (List.pairwise_mergeSort' (· ≥ ·) l)
      (pairResidual_pairwise r hqs)
  rw [firstPlayerShare, heq]
  change pickedSum (pairResidual r qs) = r + q.sum
  rw [pickedSum_pairResidual r qs 0 (by decide)]
  exact congrArg (r + ·) hqqs.sum_eq.symm

lemma firstPlayerShare_eq_half_add_residual (l q : List ℝ) (r : ℝ)
    (hl : l.Perm (r :: doubleList q)) (hsum : l.sum = 1) :
    firstPlayerShare l = (1 + r) / 2 := by
  have hpermSum := hl.sum_eq
  rw [firstPlayerShare_of_perm_residual_pairs l q r hl]
  simp only [List.sum_cons, doubleList_sum] at hpermSum
  linarith
...
lemma Refines.sum_eq {xs ys : List ℝ} (h : Refines xs ys) :
    ys.sum = xs.sum := by
  induction h with
  | nil => simp
  | cons u hu hsum hrest ih => simp [hsum, ih]
...
lemma sum_pos_of_pos_of_ne_nil (xs : List ℝ) (hne : xs ≠ [])
    (hpos : ∀ x ∈ xs, 0 < x) : 0 < xs.sum := by
  cases xs with
  | nil => exact (hne rfl).elim
  | cons x xs =>
      have hx : 0 < x := hpos x (by simp)
      have ht : 0 ≤ xs.sum := List.sum_nonneg fun z hz =>
        (hpos z (by simp [hz])).le
      simp only [List.sum_cons]
      linarith
          ...
          | cons y yt =>
              ...
              · -- Equal heads are already one matched pair.
                ...
                · subst xt
                  have hyt : yt = [] := by
                    by_contra hne
                    have hp := sum_pos_of_pos_of_ne_nil yt hne hytpos
...
lemma selectWeights_finRange_sum {m : ℕ} (g : List ℝ)
    (hlen : g.length = m) (p : Finset (Fin m)) :
    (selectWeights (List.finRange m) p (fun i => g.get (Fin.cast hlen.symm i))).sum =
      ∑ i ∈ p, g.get (Fin.cast hlen.symm i) := by
...
lemma doubleList_length (a : List ℝ) :
  ...
  induction a with
  | nil => rfl
  | cons x xs ih => simp [doubleList, ih]; omega
  ...
  have hpne : p.Nonempty := by
    ...
    · exfalso
      ...
      have hyqne : yq ≠ [] := by
        ...
        omega
      have hyqsumpos := sum_pos_of_pos_of_ne_nil yq hyqne hyqpos
...
lemma convertUpperRefines {xs ys : List ℝ}
    (h : LiuBangXiangYuUpperScratch.Refines xs ys) :
    P3Realization.Refines xs ys := by
  induction h with
  | nil => exact P3Realization.Refines.nil
  | cons u hu hsum hrest ih =>
      exact P3Realization.Refines.cons u hu hsum ih
...
lemma p3_doubleList_eq_pairDup (l : List ℝ) :
    LiuBangXiangYuUpperScratch.doubleList l = P3Agent.pairDup l := by
  rw [LiuBangXiangYuUpperScratch.doubleList_eq_flatMap]
  rfl
...
lemma realize_p3_refinement {A : Finset ℝ} {l : List ℝ}
    (href : P3Realization.Refines (P3Realization.pieceLengths A) l)
    (hlpos : ∀ z ∈ l, 0 < z) :
    ∃ B : Finset ℝ,
  ...
  have hsum : l.sum = 1 := by
    rw [href.sum_eq, P3Realization.pieceLengths_sum]
  ...
  have hne : l ≠ [] := by
    ...
    rw [hnil] at hsum
    simp at hsum
  have hpiecesP3 : P3Realization.pieceLengths (A ∪ B) = l := by
    rw [P3Realization.union_extraCuts hAcut]
    exact P3Realization.pieceLengths_cutFinset l hne hlpos hsum
  have hpieces : pieceLengths (A ∪ B) = l := by
    simpa only [pieceLengths, P3Realization.pieceLengths] using hpiecesP3
...
theorem upper_bound_aux (n : ℕ) (hn : 0 < n) :
  ...
  · let g : List ℝ := P3Realization.pieceLengths A
    have hgpos : ∀ z ∈ g, 0 < z := by
      exact P3Realization.pieceLengths_pos A hA.1
    have hgsum : g.sum = 1 := by
      exact P3Realization.pieceLengths_sum A
    have hglen : g.length = n + 1 := by
      dsimp [g]
      rw [p3_pieceLengths_length, hfull]
    have hgne : g ≠ [] := by
      intro hnil
      rw [hnil] at hglen
      simp at hglen
    ...
    have href : P3Realization.Refines (P3Realization.pieceLengths A) l := by
      exact convertUpperRefines hrefUpper
    ...
    have hpermAgent : l.Perm (P3Agent.pairDup paired ++ residual) := by
      simpa only [p3_doubleList_eq_pairDup] using hperm
    have hsharele : firstPlayerShare l ≤ paired.sum + residual.sum := by
      change P3Agent.firstPlayerShare l ≤ _
